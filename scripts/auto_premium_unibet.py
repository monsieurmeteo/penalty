...
def main():
    ...
    # ── Sélection 100% Score AdamChoi >= 80/100 & Cote Over 2.5 >= 1.40 ──
    # Critères : ac_score >= 80/100 ET over25 >= 1.40
    ...
    for r in scanned_results:
        ...
        if ac_score >= 80 and o25 is not None and o25 >= 1.40:
            ...
            if ac_score < 80:
                ...
                r["rejection_reason"] = f"Cote Over 2.5 trop basse ({o25:.2f} < 1.40)"
            else:
    ...
    nb_double = 0
    nb_simple = 0
    print(f"⭐ Matchs validés (Score >= 80/100, Cote O2.5 >= 1.40) : {len(s3_matches)} / {len(scanned_results)}")
    ...
    # ── MOTEUR COMBINÉS 100% OVER 2.5 BUTS (Cote Min: 2.20, Score >= 80, Cote O2.5 >= 1.40) ──
    # ponytail: Combinés 2 matchs Over 2.5 uniquement, pas d'Over 1.5, pas de Penalty. Cote indiv >= 1.40, combinée >= 2.20.
    TARGET_COMB    = 2.20
    MIN_COMB_ODDS  = 2.20
    MIN_O25_ODDS   = 1.40
    ...
    print(f"📊 Pool Over 2.5 (Score >= 80/100, Cote >= 1.40) : {len(pool_o25)} matchs")
        ...
        <div style="max-width:700px; margin:0 auto; background:#ffffff; border-radius:14px; overflow:hidden; box-shadow:0 4px 20px rgba(0,0,0,0.08);">
          ...
          <div style="background:#f8fafc; border-bottom:1px solid #e2e8f0; padding:14px 16px;">
            <table style="width:100%; border-collapse:collapse; text-align:center;">
              <tr>
                ...
                <td style="padding:0 4px;"><div style="background:#dbeafe; border-radius:8px; padding:10px;"><div style="font-size:24px; font-weight:900; color:#1d4ed8;">{len(s3_matches)}</div><div style="font-size:10px; font-weight:700; color:#1d4ed8;">MATCHS RETENUS</div><div style="font-size:10px; color:#3b82f6;">Score ≥ 80 · Cote ≥ 1.40</div></div></td>
          ...
          <div style="padding:12px 16px 10px 16px; background:#f8fafc; border-top:2px solid #e2e8f0;">
            <div style="font-size:14px; font-weight:800; color:#0f172a; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center;">
              <span>🎟️ COMBINÉS OVER 2.5 BUTS &nbsp;<span style="font-size:12px; font-weight:600; color:#64748b;">(Score AdamChoi ≥ 80/100 · Cote O2.5 ≥ 1.40 · Cote Min 2.20 · Mise 4€)</span></span>
    ...
    report = [
        ...
        f"**Critères** : Score AdamChoi ≥ 80/100  ET  Cote Over 2.5 ≥ 1.40  ET  Cote combinée ≥ 2.20\n",
    ...
    raw_subject = f"⚽ Football {subject_date} — {len(combos_mixed)} Combinés Over 2.5 (Cote >= 2.20) · {nb_s3} Matchs Retenus (Score >= 80, Cote >= 1.40)"
